...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.customer_service_representative?.length > 0) {
      const create_customer_service_representative = body.customer_service_representative;
      body.customer_service_representative = {
        create: create_customer_service_representative,
      };
    } else {
      delete body.customer_service_representative;
    }
    if (body?.recharge_coupon?.length > 0) {
      const create_recharge_coupon = body.recharge_coupon;
      body.recharge_coupon = {
        create: create_recharge_coupon,
      };
    } else {
      delete body.recharge_coupon;
    }
